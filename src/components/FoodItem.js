...
const FoodItem = ({ name, price, imageId, count, description, id }) => {
  const item = {
    name: name,
    imageId: imageId,
    price: price,
  ...
  return (
    <>
      <div className="border border-solid border-blue-300 w-[18%] p-3 m-[1%] h-80 shadow hover:bg-blue-50 ">
        <h1 className="font-bold">{name.length > 25 ? name.slice(0,25)+"..." : name}</h1>
        <img className="w-[100%] h-[50%]" src={IMG_CDN_URL + imageId}></img>
        <h3 class="italic">
        ...
        <div className="flex flex-row items-start  border border-blue-200 bg-blue-400  w-[50%] mx-[25%] gap-8  ">
